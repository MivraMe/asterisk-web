...
async def inbound_route_new_form(request: Request, db: AsyncSession = Depends(get_db)):
    trunks = await crud.list_trunks(db)
    return templates.TemplateResponse(request, "pages/inbound_route_form.html", {"nav_active": "inbound-routes", "route": None, "trunks": trunks})
...
async def inbound_route_new_submit(
    ...
    try:
        ...
        trunks = await crud.list_trunks(db)
        return templates.TemplateResponse(request, "pages/inbound_route_form.html", {"route": None, "form": form, "trunks": trunks, "error": _err(exc)}, status_code=400)
...
async def inbound_route_edit_form(route_id: int, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    trunks = await crud.list_trunks(db)
    return templates.TemplateResponse(request, "pages/inbound_route_form.html", {"nav_active": "inbound-routes", "route": route, "trunks": trunks})
...
async def inbound_route_edit_submit(
    ...
    try:
        ...
        route = await crud.get_inbound_route(db, route_id)
        trunks = await crud.list_trunks(db)
        return templates.TemplateResponse(request, "pages/inbound_route_form.html", {"route": route, "trunks": trunks, "error": _err(exc)}, status_code=400)
    return RedirectResponse(url="/inbound-routes", status_code=303)
...
async def outbound_route_new_submit(
    ...
    try:
        payload = OutboundRouteCreate(pattern=pattern, trunk_id=trunk_id, priority=priority, strip_digits=strip_digits, prepend=prepend or None)
